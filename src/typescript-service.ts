...
export default class TypeScriptService {
	...
	getDefinition(uri: string, line: number, column: number): Promise<Location[]> {
		console.error("# getDefinition", uri, line, column);
	...
	getReferences(uri: string, line: number, column: number): Promise<Location[]> {
		console.error("# getReferences", uri, line, column);
	...
	getWorkspaceSymbols(query: string, limit?: number): Promise<SymbolInformation[]> {
		console.error("# getWorkspaceSymbols (REALL)", query, limit);
		...
		return new Promise<SymbolInformation[]>((resolve, reject) => {
			...
			// console.error("# getWorkspaceSymbols DONE", query, limit);
			// resolve([]);
			// return;
			...
			if (!query && this.workspaceSymbols) {
				console.error("# getWorkspaceSymbols END (CACHED)", (new Date().getTime() - start) / 1000);
			...
			this.collectWorkspaceSymbols(query, limit, configurations, index, items, () => {
				...
				resolve(items);
				console.error("# getWorkspaceSymbols END", items.length, "items", (new Date().getTime() - start) / 1000);
			});
		});
	}
